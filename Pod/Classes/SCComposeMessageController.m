...
- (BOOL)textFieldShouldReturn:(UITextField *)textField;
{
    ...
    if (res) {
        [self toggleMessageView:YES];
        self.icon.image = [UIImage imageNamed:@"ico_sms-24x24"];
...
-(void) handleNotification:(NSNotification *) notification
{
    ...
    if ([[notification name] isEqualToString:@"SearchTableController:NewMessage"]) {
        ...
        if ([[[notification userInfo] objectForKey:@"Type"] isEqualToString:@"SMS"]) {
            self.icon.image = [UIImage imageNamed:@"ico_sms"];
        } else {
            self.icon.image = [UIImage imageNamed:@"ico_friendcaller-color"];
        }
        [self.messageField becomeFirstResponder];
    }
...
(ABPeoplePickerNavigationController *)peoplePicker
    ...
    self.richMessageKey = key;
    self.attachmentImage.image = [UIImage imageNamed:@"btn_ico_adressbook_contact"];
